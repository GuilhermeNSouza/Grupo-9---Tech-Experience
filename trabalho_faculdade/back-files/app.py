...
def prever_demanda_sazonal_simples(dados_vendas, periodo_previsao):
    media_demanda = dados_vendas['quantidade'].mean()
    total_previsto = max(0, round(media_demanda * periodo_previsao))
    return total_previsto
...
def desperdicio_produtos():
    try:
        ...
        # Consulta para obter produtos vencidos e a quantidade em estoque
        query = '''
            SELECT nome, estoqueAtual AS desperdicio_total, dataValidade
            FROM produto
            WHERE dataValidade < ?
            ORDER BY desperdicio_total DESC
        '''
        cursor.execute(query, (data_atual.strftime('%Y-%m-%d'),))
        produtos = cursor.fetchall()
        ...
        for produto in produtos:
            ...
            if isinstance(data_validade_raw, str):
                ...
                try:
                    data_validade = datetime.strptime(data_validade_raw, '%Y-%m-%d %H:%M:%S')
                except ValueError:
                    data_validade = datetime.strptime(data_validade_raw, '%Y-%m-%d')
            elif isinstance(data_validade_raw, (int, float)):
                # Trata como timestamp e converte para datetime
                data_validade = datetime.fromtimestamp(data_validade_raw / 1000)  # Divisão por 1000 para converter de ms para s
            else:
            ...
            resultado.append({
                ...
                'data_validade': data_validade.strftime('%d/%m/%Y')  # Converte para string formatada
            })
...
def cadastrar_produto():
    try:
        ...
        estoque_atual = dados.get('estoqueAtual')
        data_validade = dados.get('dataValidade')
        categoria_id = dados.get('categoriaId', 'categoria_default')  # Valor padrão para categoria
        data_cadastro = datetime.now().strftime('%Y-%m-%d %H:%M:%S')  # Data atual
        
        conn = connection_database()
        cursor = conn.cursor()

        # Insere o produto na tabela com o ID gerado
        query = '''
            INSERT INTO produto (id, nome, preco, estoqueAtual, dataValidade, dataCadastro, categoriaId)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        '''
        cursor.execute(query, (produto_id, nome, preco, estoque_atual, data_validade, data_cadastro, categoria_id))
        conn.commit()
        conn.close()
        
        return jsonify({'mensagem': 'Produto cadastrado com sucesso!'}), 201
    except Exception as e:
        print(f"Erro na rota /api/cadastrar-produto: {e}")
        return jsonify({'erro': str(e)}), 500
